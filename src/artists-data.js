export const artists = {
  1: {
    name: "Aimee Mann",
    albums: [
      {
        title: "I'm with Stupid",
        released: "1995",
        songs: [
          {
            title: "Long Shot",
            duration: "3:13",
          },
          {
            title: "Choice in the Matter",
            duration: "3:13",
          },
          {
            title: "Amateur",
            duration: "4:55",
          },
          {
            title: "Par for the Course",
            duration: "6:02",
          },
          {
            title: "That's Just What You Are",
            duration: "4:25",
          },
          {
            title: "Ray",
            duration: "4:50",
          },
        ],
      },
      {
        title: "Magnolia (Music from the Motion Picture)",
        released: "1999",
        songs: [
          {
            title: "Deathly",
            duration: "5:37",
          },
          {
            title: "You Do",
            duration: "3:45",
          },
          {
            title: "Save Me",
            duration: "4:34",
          },
        ],
      },
      {
        title: "Whatever",
        released: "1993",
        songs: [
          {
            title: "I've Had It",
            duration: "4:42",
          },
        ],
      },
    ],
  },
  2: {
    name: "Alanis Morissette",
    albums: [
      {
        title: "Jagged Little Pill (2015 Remaster)",
        released: "1995 (2015)",
        songs: [
          {
            title: "All I Really Want - 2015 Remaster",
            duration: "4:44",
          },
          {
            title: "Right through You - 2015 Remaster",
            duration: "2:56",
          },
          {
            title: "You Learn - 2015 Remaster",
            duration: "4:00",
          },
          {
            title: "Head over Feet - 2015 Remaster",
            duration: "4:27",
          },
          {
            title: "Not the Doctor - 2015 Remaster",
            duration: "3:46",
          },
        ],
      },
      {
        title: "Live / [MTV] Unplugged",
        released: "1999",
        songs: [
          {
            title: "No Pressure over Cappuccino - Live / Unplugged",
            duration: "4:41",
          },
          {
            title: "That I Would Be Good - Live / Unplugged",
            duration: "4:14",
          },
          {
            title: "Ironic - Live / Unplugged",
            duration: "3:54",
          },
          {
            title: "These R the Thoughts - Live / Unplugged",
            duration: "3:26",
          },
        ],
      },
    ],
  },
  3: {
    name: "Alison Krauss",
    albums: [
      {
        title: "Forget about It",
        released: "1999",
        songs: [
          {
            title: "Stay",
            duration: "3:26",
          },
          {
            title: "Forget about It",
            duration: "3:29",
          },
          {
            title: "Empty Hearts",
            duration: "3:24",
          },
          {
            title: "That Kind of Love",
            duration: "3:43",
          },
          {
            title: "Dreaming My Dreams with You",
            duration: "4:28",
          },
        ],
      },
      {
        title: "I've Got That Old Feeling",
        released: "1990",
        songs: [
          {
            title: "That Makes One of Us",
            duration: "3:24",
          },
        ],
      },
    ],
  },
  4: {
    name: "Ani DiFranco",
    albums: [
      {
        title: "Ani DiFranco (Remastered)",
        released: "1990 (1992)",
        songs: [
          {
            title: "The Story - Remastered",
            duration: "3:30",
          },
        ],
      },
      {
        title: "Living in Clip",
        released: "1997",
        songs: [
          {
            title: "Untouchable Face - bloomington, ill.",
            duration: "3:37",
          },
        ],
      },
      {
        title: "Not a Pretty Girl",
        released: "1995",
        songs: [
          {
            title: "Shy",
            duration: "4:45",
          },
        ],
      },
      {
        title: "Not So Soft",
        released: "1991",
        songs: [
          {
            title: "She Says",
            duration: "3:36",
          },
          {
            title: "Looking for Holes",
            duration: "3:33",
          },
        ],
      },
      {
        title: "Out of Range",
        released: "1994",
        songs: [
          {
            title: "Out of Range (Acoustic)",
            duration: "3:46",
          },
          {
            title: "Hell Yeah",
            duration: "5:02",
          },
          {
            title: "Overlap",
            duration: "3:45",
          },
        ],
      },
      {
        title: "Puddle Dive",
        released: "1993",
        songs: [
          {
            title: "Anyday",
            duration: "3:01",
          },
        ],
      },
      {
        title: "To the Teeth",
        released: "1997",
        songs: [
          {
            title: "I Know This Bar",
            duration: "5:32",
          },
        ],
      },
    ],
  },
  5: {
    name: "Edie Brickell",
    albums: [
      {
        title: "Picture Perfect Morning",
        released: "1994",
        songs: [
          {
            title: "Tomorrow Comes",
            duration: "3:57",
          },
          {
            title: "Green",
            duration: "3:21",
          },
          {
            title: "Hard Times",
            duration: "3:41",
          },
          {
            title: "In the Bath",
            duration: "2:44",
          },
        ],
      },
    ],
  },
  6: {
    name: "Fiona Apple",
    albums: [
      {
        title: "Tidal",
        released: "1996",
        songs: [
          {
            title: "The First Taste",
            duration: "4:47",
          },
          {
            title: "Never Is a Promise",
            duration: "5:55",
          },
          {
            title: "Pale September",
            duration: "5:51",
          },
          {
            title: "Carrion",
            duration: "5:44",
          },
        ],
      },
      {
        title: "When the Pawn...",
        released: "1999",
        songs: [
          {
            title: "A Mistake",
            duration: "4:56",
          },
          {
            title: "I Know",
            duration: "4:55",
          },
        ],
      },
    ],
  },
  7: {
    name: "Jewel",
    albums: [
      {
        title: "Pieces of You",
        released: "1995",
        songs: [
          {
            title: "Who Will Save Your Soul",
            duration: "3:59",
          },
          {
            title: "Pieces of You",
            duration: "4:16",
          },
          {
            title: "Little Sister",
            duration: "2:31",
          },
          {
            title: "Foolish Games",
            duration: "5:41",
          },
          {
            title: "Near You Always",
            duration: "3:09",
          },
          {
            title: "You Were Meant for Me",
            duration: "4:14",
          },
        ],
      },
      {
        title: "Spirit",
        released: "1998",
        songs: [
          {
            title: "Hands",
            duration: "3:55",
          },
          {
            title: "Kiss the Flame",
            duration: "3:17",
          },
        ],
      },
    ],
  },
  8: {
    name: "Juliana Hatfield",
    albums: [
      {
        title: "Forever Baby",
        released: "1992",
        songs: [
          {
            title: "Everybody Loves Me But You",
            duration: "3:35",
          },
        ],
      },
      {
        title: "I See You",
        released: "1992",
        songs: [
          {
            title: "Feed Me",
            duration: "3:12",
          },
        ],
      },
      {
        title: "Only Everything",
        released: "1995",
        songs: [
          {
            title: "Universal Heart-Beat",
            duration: "3:26",
          },
          {
            title: "Live on Tomorrow",
            duration: "2:59",
          },
          {
            title: "Bottles and Flowers",
            duration: "5:26",
          },
          {
            title: "Hang Down from Heaven",
            duration: "3:23",
          },
          {
            title: "My Darling",
            duration: "2:53",
          },
          {
            title: "Simplicity Is Beautiful",
            duration: "4:15",
          },
        ],
      },
      {
        title: "Please Do Not Disturb",
        released: "1997",
        songs: [
          {
            title: "Trying Not to Think about It",
            duration: "3:03",
          },
        ],
      },
    ],
  },
  9: {
    name: "Kristin Hersh",
    albums: [
      {
        title: "Hips and Makers",
        released: "1993",
        songs: [
          {
            title: "Teeth",
            duration: "4:10",
          },
          {
            title: "Sundrops",
            duration: "4:02",
          },
          {
            title: "Houdini Blues",
            duration: "4:27",
          },
          {
            title: "A Loon",
            duration: "4:19",
          },
          {
            title: "Velvet Days",
            duration: "3:52",
          },
          {
            title: "Me and My Charms",
            duration: "4:17",
          },
          {
            title: "Tuesday Night",
            duration: "3:03",
          },
        ],
      },
      {
        title: "Murder, Misery and Then Goodnight",
        released: "1998",
        songs: [
          {
            title: "I Never Will Marry",
            duration: "2:31",
          },
          {
            title: "Three Nights Drunk",
            duration: "2:01",
          },
        ],
      },
      {
        title: "Strings",
        released: "1994",
        songs: [
          {
            title: "The Key",
            duration: "3:36",
          },
        ],
      },
    ],
  },
  10: {
    name: "Liz Phair",
    albums: [
      {
        title: "Exile in Guyville (2018 Remaster)",
        released: "1993",
        songs: [
          {
            title: "Gunshy - 2018 Remaster",
            duration: "3:15",
          },
        ],
      },
      {
        title: "Liz Phair",
        released: "2003",
        songs: [
          {
            title: "Extraordinary",
            duration: "3:25",
          },
          {
            title: "It's Sweet",
            duration: "2:55",
          },
          {
            title: "Firewalker",
            duration: "4:29",
          },
        ],
      },
      {
        title: "Whitechocolatespaceegg",
        released: "1998",
        songs: [
          {
            title: "White Chocolate Space Egg",
            duration: "4:36",
          },
          {
            title: "Big Tall Man",
            duration: "3:49",
          },
          {
            title: "Perfect World",
            duration: "2:15",
          },
          {
            title: "Go On Ahead",
            duration: "2:53",
          },
          {
            title: "Headache",
            duration: "2:53",
          },
          {
            title: "Ride",
            duration: "3:04",
          },
        ],
      },
    ],
  },
  11: {
    name: "Melanie Doane",
    albums: [
      {
        title: "Adam's Rib",
        released: "1998",
        songs: [
          {
            title: "Absolutely Happy",
            duration: "3:27",
          },
          {
            title: "I Can't Take My Eyes off You",
            duration: "3:55",
          },
          {
            title: "The Space between Us",
            duration: "5:03",
          },
        ],
      },
    ],
  },
  12: {
    name: "Michelle Branch",
    albums: [
      {
        title: "Broken Bracelet",
        released: "2000",
        songs: [
          {
            title: "Washing Machine",
            duration: "2:54",
          },
          {
            title: "I'd Rather Be in Love",
            duration: "3:51",
          },
          {
            title: "Paper Pieces",
            duration: "3:34",
          },
          {
            title: "Stewart's Coat",
            duration: "3:25",
          },
          {
            title: "Goodbye to You",
            duration: "4:26",
          },
          {
            title: "Sweet Misery (Acoustic)",
            duration: "3:11",
          },
          {
            title: "All You Wanted",
            duration: "3:38",
          },
        ],
      },
      {
        title: "The Spirit Room",
        released: "2001",
        songs: [
          {
            title: "All You Wanted",
            duration: "3:38",
          },
        ],
      },
    ],
  },
  13: {
    name: "Paula Cole",
    albums: [
      {
        title: "Harbinger",
        released: "1993",
        songs: [
          {
            title: "Watch the Woman's Hands",
            duration: "4:22",
          },
          {
            title: "Bethlehem",
            duration: "4:39",
          },
        ],
      },
      {
        title: "This Fire",
        released: "1996",
        songs: [
          {
            title: "Carmen",
            duration: "3:45",
          },
          {
            title: "Me",
            duration: "5:01",
          },
          {
            title: "I Don't Want to Wait",
            duration: "5:20",
          },
        ],
      },
    ],
  },
  14: {
    name: "Paula Cole, Mick Guzauski, Roger Moutenot",
    albums: [
      {
        title: "Amen",
        released: "1999",
        songs: [
          {
            title: "Free",
            duration: "3:55",
          },
        ],
      },
    ],
  },
  15: {
    name: "Sarah McLachlan",
    albums: [
      {
        title: "The Freedom Sessions",
        released: "1994",
        songs: [
          {
            title: "Mary",
            duration: "3:57",
          },
        ],
      },
      {
        title: "Fumbling towards Ecstasy",
        released: "1993",
        songs: [
          {
            title: "Plenty",
            duration: "4:05",
          },
          {
            title: "Good Enough",
            duration: "5:04",
          },
          {
            title: "Elsewhere",
            duration: "4:44",
          },
          {
            title: "Ice",
            duration: "3:54",
          },
        ],
      },
      {
        title: "Mirrorball",
        released: "1999",
        songs: [
          {
            title: "I Love You",
            duration: "4:30",
          },
        ],
      },
      {
        title: "Solace",
        released: "1991",
        songs: [
          {
            title: "Black",
            duration: "5:06",
          },
        ],
      },
      {
        title: "Surfacing",
        released: "1997",
        songs: [
          {
            title: "Sweet Surrender",
            duration: "4:01",
          },
          {
            title: "Do What You Have to Do",
            duration: "3:47",
          },
          {
            title: "Black & White",
            duration: "5:02",
          },
        ],
      },
    ],
  },
  16: {
    name: "Sheryl Crow",
    albums: [
      {
        title: "Sheryl Crow",
        released: "1996",
        songs: [
          {
            title: "A Change Would Do You Good",
            duration: "3:50",
          },
          {
            title: "Home",
            duration: "4:51",
          },
          {
            title: "If It Makes You Happy",
            duration: "5:23",
          },
          {
            title: "Oh Marie",
            duration: "3:30",
          },
        ],
      },
      {
        title: "The Globe Sessions",
        released: "1998",
        songs: [
          {
            title: "Riverwide",
            duration: "4:07",
          },
          {
            title: "It Don't Hurt",
            duration: "4:50",
          },
        ],
      },
      {
        title: "Tuesday Night Music Club",
        released: "1993",
        songs: [
          {
            title: "Leaving Las Vegas",
            duration: "5:09",
          },
          {
            title: "No One Said It Would Be Easy",
            duration: "5:29",
          },
          {
            title: "What I Can Do for You",
            duration: "4:15",
          },
        ],
      },
    ],
  },
  17: {
    name: "Sinéad O'Connor",
    albums: [
      {
        title: "Am I Not Your Girl?",
        released: "1992",
        songs: [
          {
            title: "Don't Cry for Me Argentina",
            duration: "5:35",
          },
          {
            title: "Gloomy Sunday",
            duration: "3:54",
          },
        ],
      },
      {
        title: "Gospel Oak",
        released: "1997",
        songs: [
          {
            title: "This Is to Mother You",
            duration: "3:13",
          },
          {
            title: "Petit Poulet",
            duration: "3:44",
          },
          {
            title: "4 My Love",
            duration: "4:05",
          },
        ],
      },
      {
        title: "I Do Not Want What I Haven't Got",
        released: "1990",
        songs: [
          {
            title: "You Cause as Much Sorrow",
            duration: "5:03",
          },
          {
            title: "I Do Not Want What I Haven't Got",
            duration: "5:48",
          },
          {
            title: "Jump in the River",
            duration: "4:12",
          },
        ],
      },
      {
        title: "Universal Mother",
        released: "1994",
        songs: [
          {
            title: "John I Love You",
            duration: "5:32",
          },
          {
            title: "A Perfect Indian",
            duration: "4:23",
          },
          {
            title: "In This Heart",
            duration: "3:11",
          },
        ],
      },
    ],
  },
  18: {
    name: "Suzanne Vega",
    albums: [
      {
        title: "99.9 F",
        released: "1992",
        songs: [
          {
            title: "In Liverpool",
            duration: "4:41",
          },
          {
            title: "Blood Sings",
            duration: "3:18",
          },
          {
            title: "Bad Wisdom",
            duration: "3:24",
          },
          {
            title: "Song of Sand",
            duration: "3:05",
          },
        ],
      },
      {
        title: "Days of Open Hand",
        released: "1990",
        songs: [
          {
            title: "Institution Green",
            duration: "6:16",
          },
        ],
      },
      {
        title: "Nine Objects of Desire",
        released: "1996",
        songs: [
          {
            title: "Caramel",
            duration: "2:53",
          },
          {
            title: "No Cheap Thrill",
            duration: "3:09",
          },
          {
            title: "My Favorite Plum",
            duration: "2:48",
          },
        ],
      },
      {
        title: "Songs in Red and Gray",
        released: "2001",
        songs: [
          {
            title: "(I'll Never Be) Your Maggie May",
            duration: "3:47",
          },
          {
            title: "Soap and Water",
            duration: "3:03",
          },
        ],
      },
    ],
  },
  19: {
    name: "Tanya Donelly",
    albums: [
      {
        title: "Beautysleep",
        released: "2001",
        songs: [
          {
            title: "The Storm",
            duration: "4:03",
          },
          {
            title: "The Night You Saved My Life",
            duration: "3:59",
          },
          {
            title: "Keeping You",
            duration: "4:20",
          },
          {
            title: "Another Moment",
            duration: "4:34",
          },
          {
            title: "Darkside",
            duration: "3:50",
          },
          {
            title: "So Much Song",
            duration: "4:38",
          },
        ],
      },
      {
        title: "Lovesongs for Underdogs",
        released: "1997",
        songs: [
          {
            title: "Pretty Deep",
            duration: "4:23",
          },
          {
            title: "Lantern",
            duration: "3:11",
          },
          {
            title: "Goat Girl",
            duration: "2:16",
          },
          {
            title: "Swoon",
            duration: "4:28",
          },
        ],
      },
    ],
  },
  20: {
    name: "Tori Amos",
    albums: [
      {
        title: "Little Earthquakes",
        released: "1991",
        songs: [
          {
            title: "Winter",
            duration: "5:42",
          },
          {
            title: "Happy Phantom",
            duration: "3:17",
          },
          {
            title: "China",
            duration: "5:00",
          },
        ],
      },
      {
        title: "Little Earthquakes (2015 Remaster)",
        released: "1991 (2015)",
        songs: [
          {
            title: "Girl - 2015 Remaster",
            duration: "4:07",
          },
          {
            title: "Silent All These Years - 2015 Remaster",
            duration: "4:11",
          },
        ],
      },
      {
        title: "Strange Little Girls",
        released: "2001",
        songs: [
          {
            title: "Strange Little Girl",
            duration: "3:49",
          },
          {
            title: "Heart of Gold",
            duration: "4:00",
          },
        ],
      },
      {
        title: "Under the Pink (2015 Remaster",
        released: "1993 (2015)",
        songs: [
          {
            title: "God - 2015 Remaster",
            duration: "3:56",
          },
          {
            title: "Baker Baker - 2015 Remaster",
            duration: "3:20",
          },
        ],
      },
    ],
  },
  21: {
    name: "Tracy Chapman",
    albums: [
      {
        title: "Crossroads",
        released: "1989",
        songs: [
          {
            title: "Be Careful of My Heart",
            duration: "4:42",
          },
          {
            title: "This Time",
            duration: "3:44",
          },
        ],
      },
      {
        title: "Matters of the Heart",
        released: "1992",
        songs: [
          {
            title: "Open Arms",
            duration: "4:35",
          },
        ],
      },
      {
        title: "New Beginning",
        released: "1995",
        songs: [
          {
            title: "Heaven's Here on Earth",
            duration: "5:24",
          },
          {
            title: "At This Point in My Life",
            duration: "5:09",
          },
          {
            title: "The Rape of the World",
            duration: "7:08",
          },
          {
            title: "Tell It Like It Is",
            duration: "6:05",
          },
          {
            title: "Remember the Tinman",
            duration: "5:45",
          },
        ],
      },
      {
        title: "Tracy Chapman",
        released: "1988",
        songs: [
          {
            title: "For You",
            duration: "3:09",
          },
        ],
      },
    ],
  },
};
